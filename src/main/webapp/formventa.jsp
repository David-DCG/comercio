        ...
        <div class="container ">
            ...
            <div class="container col-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="MainVenta" method="post">
                            <input type="hidden" name="id" value="<%= venta.getId()%>">
                            <div class="mb-3">
                                <label for="usuario" class="form-label">Usuario:</label>
                                <select name="id_usr" class="form-control">
                                    <option value="">-- Seleccione --</option>
                                    <%
                                        for (Usuario item : usuarios) {
                                    %>
                                    <option value="<%= item.getId()%>" <%= (item.getId() == venta.getIdUsr().getId()) ? "selected" : ""%>><%= item.getNombres()%></option>
                                    <%
                                        }
                                    %>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="id_com" class="form-control">
                                    ...
                                    %>
                                    <option value="<%= item.getId()%>" <%= (item.getId() == venta.getIdCom().getId()) ? "selected" : ""%>><%= item.getNombre()%></option>
                                    <%
                                        }
                                    %>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="id_per" class="form-control">
                                    ...
                                    %>
                                    <option value="<%= item.getId()%>" <%= (item.getId() == venta.getIdPer().getId()) ? "selected" : ""%>><%= item.getNombre()%></option>
                                    <%
                                        }
                                    %>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="fecha" id="fecha" value="<%= venta.getFecha()%>" class="form-control" placeholder="">
                            </div>
                            <button type="submit" class="btn btn-success"><i class="bi bi-floppy"> Enviar</i></button>
                            <a href="MainVenta" class="btn btn-info"><i class="bi bi-arrow-return-left"> Volver</i></a>
                        </form>
                    </div>
                </div>
            </div>
